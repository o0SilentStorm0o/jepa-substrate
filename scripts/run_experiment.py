...
Runs the full 120-run grid:
  10 seeds x 3 masking policies x 2 substrates (primary) = 60 runs
  10 seeds x 3 ablation conditions x 2 substrates = 60 runs
...
def run_single(
    ...
    if substrate == "ann":
        ...
        # Measure via Lava
        from snn.lava_inference import run_snn_measurement
...
def main() -> None:
    ...
    parser.add_argument(
        "--data-dir",
        type=str,
        default=str(PROJECT_ROOT / "data" / "uci_har"),
    ...
    # Build and execute run grid
    runs = build_run_grid(cfg, args.subset)
    logger.info("Total runs: %d", len(runs))

    summaries = []
    for i, run_spec in enumerate(runs):
        logger.info("--- Run %d/%d ---", i + 1, len(runs))
        try:
            summary = run_single(run_spec, cfg, data_dir, output_root)
            summaries.append(summary)
        except Exception:
            logger.exception(
                "Run FAILED: seed=%d, policy=%s, substrate=%s",
                run_spec["seed"], run_spec["masking_policy"],
                run_spec["substrate"],
            )
            summaries.append({
                "run_id": f"s{run_spec['seed']}_{run_spec['masking_policy']}_"
